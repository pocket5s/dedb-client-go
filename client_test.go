...
import (
	...
	dedb "github.com/pocket5s/dedb-client-go/api"
...
func TestEventRead(t *testing.T) {
	...
	log := log.With().Str("logger", "TestHarness").Logger()
	eventChan := make(chan *dedb.Event, 1)
	config := ClientConfig{
		Server:        ":50001",
	...
	pool, err := newPool(false, config, &log)
	if err != nil {
		panic(err)
	}
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			log.Debug().Msgf("added event to stream %s", result)
			c.Connect(context.Background())

			time.Sleep(1 * time.Second)
			log.Debug().Msg("adding 2nd message to stream")
			result, err = pool.XAdd(ctx, &args).Result()
			if err != nil {
				panic(err)
			}
